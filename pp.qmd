...
## Proof of interarrival times property {.scrollable}

::: {.callout-tip}
## Property 3

The interarrival times $X_k \sim \text{Exp}(\lambda)$, for $k \in \{1,2,\ldots\}$
:::

::: {.fragment}

For times $0 < s \leq t$ and arrival index $i \in \{1,2,\ldots\}$ consider 
...
::: {.fragment}
$= \mathbb{P}(N(s+t) = N(s) \mid N(s) = i-1)$
:::
...
::: {.fragment}
$= \mathbb{E}_{S}[\mathbb{P}(X_{i} > t \mid T_{i-1}=S)] = e^{-\lambda t}$
:::
