...
Q1. Which elements of the Poisson Process definition are required to prove PPP1?
...
Q1. Which elements of the Poisson Process definition are required to prove PPP2?
...
with rate $\theta > 0$ if it has probability density function

$$
q_\theta(x) = \theta e^{-\theta x}, \quad x \in \mathbb{R}_{+}.
$$
...
::: {.callout-tip}
## Property 1 (EDP1)
For some random variable $X\in \mathbb{R}_{+}$, if $\mathbb{P}(X>t) = e^{-\theta t}$ for all $t \geq 0$ 

then $X \sim \text{Exp}(\theta)$.
:::
:::
...
$$\mathbb{P}(A) = \int_{\mathsf{B}} \mathbb{P}(A\mid B = b) f(b)\mathrm{d} b$$

where f(b) is the probability density function of $B$ at $b$.
:::
...
::: {.fragment}
$= \int_{\mathsf{T}_{i-1}}\mathbb{P}(X_{i} > t \mid T_{i-1}=s)f(s)\mathrm{d}s$
:::
